...
test("download website has current Mac and Windows links", () => {
  ...
  assert.match(html, /Current version: <strong>1\.3\.3<\/strong>/);
  assert.match(html, /releases\/download\/v1\.3\.3\/ChatMonJA-1\.3\.3-mac-arm64-unsigned\.dmg/);
  assert.match(html, /releases\/download\/v1\.3\.3\/ChatMonJA-1\.3\.3-windows-x64-unsigned\.zip/);
  assert.doesNotMatch(html, /releases\/latest\/download/);
  assert.match(html, /ChatMonJA-1\.3\.3-mac-arm64-unsigned\.dmg/);
  assert.match(html, /ChatMonJA-1\.3\.3-windows-x64-unsigned\.zip/);
  assert.match(html, /Elmore “JimmyQ” Jamieson/);
  assert.match(html, /href="mailto:support@chatmonja\.com">support@chatmonja\.com/);
  ...
  assert.match(script, /api\.github\.com\/repos\/jimmyqja\/ChatMonJA\/releases\/tags\/v1\.3\.3/);
  assert.match(script, /download_count/);
  assert.match(script, /ChatMonJA-1\.3\.3-mac-arm64-unsigned\.dmg/);
  assert.match(script, /ChatMonJA-1\.3\.3-windows-x64-unsigned\.zip/);
});
...
test("website includes privacy, FAQ, terms, and contact pages", () => {
  ...
  assert.match(privacy, /support@chatmonja\.com/);
  ...
  assert.match(contact, /mailto:support@chatmonja\.com/);
  ...
  for (const page of [privacy, faq, terms, contact]) {
    ...
    assert.match(page, /mailto:support@chatmonja\.com/);
