...
test("Commands Manager UI and secure bridge channels are included", () => {
  ...
  assert.match(html, /id="raidTab"/);
  assert.match(html, /id="defaultRaidChannel"/);
  assert.match(html, /id="manualRaidChannel"/);
  assert.match(html, /saveRaidChannel/);
  assert.match(html, /startManualRaid/);
  assert.match(html, /id="updateStatus"/);
  assert.match(html, /checkUpdates/);
  assert.match(html, /openUpdateDownload/);
  assert.match(html, /10 Jamaican greeting presets/);
  assert.match(html, /ChatMonJA v1\.3\.4/);
  ...
  assert.match(preload, /"check-updates"/);
  assert.match(preload, /"open-update-download"/);
  assert.match(preload, /"save-raid-channel"/);
  assert.match(preload, /"clear-raid-channel"/);
  assert.match(preload, /"start-raid"/);
  ...
  assert.match(preload, /"raid-settings"/);
